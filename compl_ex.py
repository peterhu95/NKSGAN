...
class ComplExModule(BaseModule):
    ...
    def forward(self, src, rel, dst,gat_flag=False,h_prime=0):
        if not gat_flag:
            ...
            n=self.rel_re_embed.weight.size()[1]
            h_prime_ent_re_embed = h_prime[:,:n]
            h_prime_ent_im_embed = h_prime[:,n:2*n]
            return t.sum(self.rel_re_embed(rel) * h_prime_ent_re_embed[src] * h_prime_ent_re_embed[dst], dim=-1) \
                + t.sum(self.rel_re_embed(rel) * h_prime_ent_im_embed[src] * h_prime_ent_im_embed[dst], dim=-1) \
                + t.sum(self.rel_im_embed(rel) * h_prime_ent_re_embed[src] * h_prime_ent_im_embed[dst], dim=-1) \
                - t.sum(self.rel_im_embed(rel) * h_prime_ent_im_embed[src] * h_prime_ent_re_embed[dst], dim=-1)
    ...
    def prob_logit(self, src, rel, dst, gat_flag=False, h_prime=0):
        if not gat_flag:
            ...
            return self.forward(src, rel, dst,gat_flag,h_prime)
