...
class TransDModule(BaseModule):
    ...
    #def get_proj_ent_embedding(self):  # 还没想好，因为计算实体的时候要与关系对应上，然后做映射，那就换一种实现思路。
    #    n_rel = self.rel_embed.weight.size()[0]
    #    n_ent = self.ent_embed.weight.size()[0]
    #    ent_idx = t.arange(n_ent)
    #    rel_idx = t.arange(n_rel)
    #    ent_proj = self.ent_embed(ent_idx.cuda()) +\
    #               t.sum(self.proj_ent_embed(ent_idx.cuda()) * self.ent_embed(ent_idx.cuda()), dim=-1, keepdim=True) * self.proj_rel_embed(rel_idx.cuda())
    #    return ent_proj
    ...
    def forward(self, src, rel, dst,gat_flag=False,h_prime=0):
        if not gat_flag:
            src_proj = self.ent_embed(src) +\
                   t.sum(self.proj_ent_embed(src) * self.ent_embed(src), dim=-1, keepdim=True) * self.proj_rel_embed(rel)
            dst_proj = self.ent_embed(dst) +\
            ...
            src_proj = h_prime[src] +\
                   t.sum(self.proj_ent_embed(src) * h_prime[src], dim=-1, keepdim=True) * self.proj_rel_embed(rel)
            dst_proj = h_prime[dst] +\
                   t.sum(self.proj_ent_embed(dst) * h_prime[dst], dim=-1, keepdim=True) * self.proj_rel_embed(rel)
            return t.norm(dst_proj - self.rel_embed(rel) - src_proj + 1e-30, p=self.p, dim=-1)
    ...
    def prob_logit(self, src, rel, dst, gat_flag=False, h_prime=0):
        if not gat_flag:
            return -self.forward(src, rel ,dst) / self.temp
        else:
            return -self.forward(src, rel ,dst,gat_flag,h_prime) / self.temp
