...
def get_bern_prob(data, n_ent, n_rel):
    ...
    for s, r, t in zip(src, rel, dst):  # s: tensor(1437) r: tensor(3) t: tensor(1435)
...
class BernCorrupterMulti(object):
    ...
    def corrupt(self, src, rel, dst, keep_truth=True):
        ...
        return torch.from_numpy(src_out), rel_out, torch.from_numpy(dst_out), torch.from_numpy(selection.astype('int'))
